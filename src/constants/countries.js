/**
 * Every country the phone field offers, with its calling code.
 *
 * GENERATED, once, from the data the SohozKaj website already registers people
 * with: calling codes from `libphonenumber-js/metadata.min.json` and names from
 * `react-phone-number-input/locale/en.json` (both in `sohozkaj-app`). Neither
 * library is a dependency here — this app needs the list and the dial code, not
 * a parser, and the server validates the finished number anyway.
 *
 * Names stay English in both locales, the same rule template titles follow:
 * they are proper nouns, and the library ships no Bengali list to pick from.
 *
 * Flags are not stored. A flag emoji *is* the country code — two regional
 * indicator letters — so `flagOf` derives it instead of shipping 245 of them.
 */
...
export const COUNTRIES = [
  ...
  { code: "AC", name: "Ascension Island", dial: "247" },
  ...
  { code: "VA", name: "Holy See (Vatican City State)", dial: "39" },
...
/** Bengali-first product, Bangladeshi users: the default, and the SMS path. */
export const DEFAULT_COUNTRY = 'BD';

const BY_CODE = new Map(COUNTRIES.map((c) => [c.code, c]));

export const countryOf = (code) => BY_CODE.get(code) || BY_CODE.get(DEFAULT_COUNTRY);

/** 'BD' → '+880'. */
export const dialOf = (code) => `+${countryOf(code).dial}`;

/** 'BD' → 🇧🇩. Regional indicators sit 0x1F1E6 above 'A'. */
